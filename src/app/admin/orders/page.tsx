        ...
        {(["all", ...STATUS_OPTIONS] as const).map((s) => (
          <button
            ...
            className={`px-3 py-2 text-xs font-semibold rounded-xl capitalize transition-colors ${
              filterStatus === s ? "bg-black text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gray-400"
            }`}
          >
            {s}
            {s !== "all" && (
                            ...
                            <div className="space-y-1">
                              {order.items.map((item, i) => (
                                <div key={i} className="flex justify-between items-center text-sm">
                                  ...
                                  <span className="font-semibold text-gray-900">{fmt(item.unitPrice * item.quantity)}</span>
                                </div>
                              ))}
