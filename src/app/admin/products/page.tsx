...
export default function AdminProductsPage() {
  const { products, deleteProduct, toggleFeatured, toggleActive } = useAdminStore();
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<"all" | "active" | "inactive" | "featured" | "soldout">("all");
  ...
  const filtered = products.filter((p) => {
    ...
    const matchFilter =
      ...
      filter === "soldout"  ? p.stock_quantity === 0 : true;
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div className="flex gap-1.5">
          {(["all", "active", "inactive", "featured", "soldout"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-2 text-xs font-semibold rounded-xl capitalize transition-colors ${
                filter === f ? "bg-black text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gray-400"
              }`}
            >
                ...
                return (
                  <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${!p.is_active ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-5 py-3 text-right">
                      ...
                      {p.compare_at_price && (
                        <p className="text-xs text-gray-400 line-through">{fmt(p.compare_at_price)}</p>
                      )}
                    </td>
                    {/* Stock */}
                    <td className="px-5 py-3 text-right">
                      <span className={`text-sm font-bold ${p.stock_quantity === 0 ? "text-red-500" : p.stock_quantity < 5 ? "text-orange-500" : "text-gray-900"}`}>
                        {p.stock_quantity}
                      </span>
                    </td>
                    {/* Active toggle */}
