...
const inputCls =
  "w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-gray-400 transition-colors bg-white";
...
export function SettingsClient({
  ...
  /* ── Hero local state ── */
  const [heroForm, setHeroForm] = useState<HeroData>({ ...initialHero });
  const [heroSaved, setHeroSaved] = useState(false);

  const saveHero = async () => {
    await updateHero(heroForm as unknown as Record<string, unknown>);
    setHeroSaved(true);
    setTimeout(() => setHeroSaved(false), 2000);
  };

  /* ── Site Settings local state ── */
  const [siteForm, setSiteForm] = useState<SiteData>({ ...initialSite });
  const [siteSaved, setSiteSaved] = useState(false);

  const saveSite = async () => {
    await updateSiteSettings(siteForm as unknown as Record<string, unknown>);
    setSiteSaved(true);
  ...
  const [messages, setMessages] = useState<string[]>([
    ...initialAnnouncement.messages,
  ]);
  const [announcementSpeed, setAnnouncementSpeed] = useState(
    initialAnnouncement.speed
  );
  ...
  const saveMsgs = async () => {
    await updateAnnouncement(messages, announcementSpeed);
    setMsgSaved(true);
    ...
    <div className="space-y-6 max-w-3xl">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-0.5">
          Control every aspect of your storefront.
        </p>
      </div>
      ...
      <Section title="Hero Banner">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setHeroForm((f) => ({ ...f, ctaLabel: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setHeroForm((f) => ({ ...f, ctaHref: e.target.value }))
                }
              />
            </div>
          </div>
          <div>
            ...
            <input
              className={inputCls}
              value={heroForm.imageSrc}
              onChange={(e) =>
                setHeroForm((f) => ({ ...f, imageSrc: e.target.value }))
              }
              placeholder="https://…"
            />
          </div>
          <div>
            {label(
              `Overlay Opacity: ${Math.round(heroForm.overlayOpacity * 100)}%`
            )}
            <input
              type="range"
              min={0}
          ...
          {heroForm.imageSrc && (
            <div className="relative rounded-xl overflow-hidden h-36 mt-2">
              ...
              <div
                ...
                style={{
                  background: `rgba(0,0,0,${heroForm.overlayOpacity})`,
                }}
              />
      ...
      <Section title="Announcement Bar">
        <p className="text-xs text-gray-400 -mt-2">
          Messages rotate in the top banner.
        </p>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Each message shows for this long
          </p>
        </div>
        <div className="flex justify-end pt-1">
          <button
      ...
      <Section title="Site Settings">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSiteForm((f) => ({ ...f, siteName: e.target.value }))
              }
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setSiteForm((f) => ({ ...f, siteTagline: e.target.value }))
              }
            />
          </div>
        </div>
        <div>
          ...
          />
          <p className="text-xs text-gray-400 mt-1">
            = ${(siteForm.freeShippingThreshold / 100).toFixed(2)}
          </p>
        </div>
        <div className="flex justify-end pt-1">
          <button
